...
\end{equation}
Thus a homogeneous finite-boundary scalar model can be rejected directly in multiplier space if the product has nonzero phase or changes with RF, before any spatial logarithm is taken.
...
\end{equation}
where an energy gradient in eV/m has the same numerical value as V/m for a singly charged carrier. The inherited empirical field-rolloff sensitivity law used for the field-driven velocity is
\begin{equation}
...
\label{eq:hg_vfield}
\end{equation}
Despite the historical symbol $E_{\mathrm{sat}}$, Eq.~\eqref{eq:hg_vfield} is \emph{not} an asymptotically saturating velocity law when $r_s>1$; it is retained only to reproduce the stated numerical sensitivity stress. The sampled grading-field scale is approximately $(4.1$--$4.5)\times10^4~\mathrm{V/m}$, so $E_g^{\mathrm{grad}}/E_{\mathrm{sat}}\simeq0.05$ and the rolloff changes the low-field linear velocity by only about $0.15$--$0.18\%$. The reported closure values are therefore effectively a low-field construction rather than a claim about high-field velocity saturation. The reduced density-of-states correction assumes $N_c\propto(m^*)^{3/2}$ and $m^*\propto E_g$, giving
\begin{equation}
