...
def common_profile(a,n=101):
    z=np.linspace(0.,1.,a.size); zz=np.linspace(0.,1.,n)
    return np.interp(zz,z,np.asarray(a,float))


def solve_weighting(nx,nz,p):
    dx,dz=p.width_m/nx,p.thickness_m/nz; xx=op.xs(nx,p)
    ...
    A=A.tocsr(); phi=spsolve(A,b); rr=relres(A,phi,b)
...
def mesh_case(nx,nz,p):
    psi,n,h=op.solve(nx,nz,p); c=op.currents(nx,nz,psi,n,p); f=h[-1]
    ...
    phi,wd=solve_weighting(nx,nz,p); ic=nx//2
...
def frozen_operators(psi,phi,p):
    """Assemble F from continuity and Q independently from SG jump rates."""
    nz,nx=psi.shape; dx,dz=p.width_m/nx,p.thickness_m/nz; xx=op.xs(nx,p); vol=dx*dz
    Ac,_=op.continuity(nx,nz,psi,p)
    F=(-Ac/(op.Q*vol)).tocsr()
    Qm=lil_matrix((nx*nz,nx*nz),dtype=float)
    bsel=np.zeros(nx*nz); bother=np.zeros(nx*nz)

    def jump(r,rn,pi,pj,area,dist):
        delta=(pj-pi)/p.vt
        rate=p.D*area/(dist*vol)*op.B(-delta)
        Qm[r,r]-=rate; Qm[r,rn]+=rate

    for j in range(nz):
      for i,x in enumerate(xx):
        r=op.k(j,i,nx); pi=float(psi[j,i])
        if i>0: jump(r,op.k(j,i-1,nx),pi,float(psi[j,i-1]),dz,dx)
        if i+1<nx: jump(r,op.k(j,i+1,nx),pi,float(psi[j,i+1]),dz,dx)
        if j>0: jump(r,op.k(j-1,i,nx),pi,float(psi[j-1,i]),dx,dz)
        else:
            d=(0.-pi)/p.vt; rate=p.D*dx/(.5*dz*vol)*op.B(-d)
            Qm[r,r]-=rate; bother[r]+=rate
        if j+1<nz: jump(r,op.k(j+1,i,nx),pi,float(psi[j+1,i]),dx,dz)
        elif op.contacted(float(x),p):
            d=(p.top_V-pi)/p.vt; rate=p.D*dx/(.5*dz*vol)*op.B(-d)
            Qm[r,r]-=rate; bsel[r]+=rate
    Q=Qm.tocsr()

    diff=(F-Q.transpose()).tocsr()
    op_mismatch=float(np.linalg.norm(diff.data)/max(np.linalg.norm(F.data),np.finfo(float).tiny))
    if op_mismatch>=1e-12: raise AssertionError(f'independent F/Q transpose mismatch: {op_mismatch}')
    row_loss=np.asarray(-Q.sum(axis=1)).ravel(); boundary_rate=bsel+bother
    ...
    return Q,F,bsel,bother,qramo,dict(forward_backward_operator_relative_mismatch=op_mismatch,boundary_rate_decomposition_relative_error=deficit)


def signal_gate(case,p):
    psi=case['psi']; phi=case['phi']; nz,nx=psi.shape
    Q,F,bsel,bother,qramo,diag=frozen_operators(psi,phi,p); nstate=nx*nz
    psel=spsolve((-Q).tocsc(),bsel); rp=relres((-Q).tocsr(),psel,bsel)
    h0=spsolve((-Q).tocsc(),qramo); rh0=relres((-Q).tocsr(),h0,qramo)
    ...
    x=op.xs(nx,p); z=(np.arange(nz)+.5)*p.thickness_m/nz; X,Z=np.meshgrid(x,z)
