...
def robust_fast_refit(y, baseline_r):
    starts=(
        ...
        np.asarray((baseline_r.real+PERT,baseline_r.imag),float),
        np.asarray((baseline_r.real-PERT,baseline_r.imag),float),
        np.asarray((baseline_r.real,baseline_r.imag+PERT),float),
        np.asarray((baseline_r.real,baseline_r.imag-PERT),float),
    )
    best=None
    for x0 in starts:
        x0=np.minimum(np.maximum(x0,boot.ROOT_LOWER+1e-10),boot.ROOT_UPPER-1e-10)
        opt=least_squares(lambda x: boot.profiled_residual(y,x),x0,bounds=(boot.ROOT_LOWER,boot.ROOT_UPPER),xtol=1e-10,ftol=1e-10,gtol=1e-10,max_nfev=80)
        r=boot.profiled_residual(y,opt.x); n2=float(np.dot(r,r))
        if best is None or n2<best[0]: best=(n2,r,complex(float(opt.x[0]),float(opt.x[1])))
...
def main():
    ...
    for c in gate['fast_refit_spot_checks_at_analytic_snr'].values():
        if c['max_fast_over_full_residual_norm_ratio']>1.001: raise AssertionError('repaired fast/full refit mismatch')
    out={'schema':'paper03-second-geometry-bootstrap-500-repair-v1','status':'LOCKED COPLANAR 500-MHZ BOOTSTRAP OPTIMIZER REPAIR / NON-CLAIM','repair_scope':'optimizer starts/tolerances only; statistical seeds/populations/SNR grid/alpha/power rule unchanged','optimizer':{'starts':'baseline plus +/-0.03 real and +/-0.03 imaginary','xtol':1e-10,'ftol':1e-10,'gtol':1e-10,'max_nfev':80},'original_failed_job':95543826151,'grid':[161,121],'source_quadrature':17,'forward_diagnostics':diag,'bootstrap':{'alpha':boot.ALPHA,'target_power':boot.TARGET_POWER,'n_null':boot.N_NULL,'n_alt':boot.N_ALT,'snr_offsets_db':list(boot.SNR_OFFSETS_DB)},'rf_gate':gate,'science_interpretation_ready':False}
    Path('paper03_second_geometry_bootstrap_500_repair.json').write_text(json.dumps(resolvent.json_safe(out),indent=2,allow_nan=False)+'\n')
