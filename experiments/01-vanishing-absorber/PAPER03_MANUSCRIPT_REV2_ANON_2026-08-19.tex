...
\author{Anonymous}
\date{Draft: August 19, 2026 -- Revision 2}

\begin{document}
\maketitle

\begin{abstract}
Frequency-dependent photodetector signals are often interpreted by fitting a low-dimensional transport model and assigning the fitted parameters to microscopic drift, diffusion, recombination, or transit physics. We test a distinct failure mode: ordinary multidimensional detector physics can generate a spectral/RF signature of the same order as a targeted microscopic transport signal even when the assumed homogeneous transport model is wrong. The practical question is therefore not only whether a confound exists, but whether the data reject the inadequate model before the false microscopic parameter becomes statistically defensible. We study that precision ordering with six calibrated optical generation kernels and Shockley--Ramo terminal current. A calibrated-kernel one-mode null, a two-mode diagnostic, and a cross-frequency physical root law are applied without revealing the hidden generating mechanism to the inverse. In a predeclared 60-row finite-contact/electrostatic/diffusion/recombination screen, 42 of 180 nonzero-frequency rows contain order-one confounds and none is an analytic hidden-risk row. Finite-sample parametric bootstraps at the nominal, maximum-confound, and warning-boundary coordinates reject the inadequate one-mode interpretation before the corresponding false transport claim in all nine tested RF comparisons; the smallest conservative warning margin is 9.16 dB. A materially different coplanar-contact/insulating-bottom topology gives warning margins of 52.86, 39.20, and 40.55 dB at 100 MHz, 500 MHz, and 1 GHz. Finally, a coupled Poisson--Scharfetter--Gummel operating-state calculation with independent weighting-potential and forward/backward signal checks passes refined numerical gates. Its aligned-mesh six-current observable converges, and a blind calibrated-kernel analysis rejects the one-mode interpretation far below the frozen false-claim precision. The historical raw four-color phase is ill-conditioned in this final self-consistent case and is reported only as a diagnostic, not used as quantitative evidence for the warning margin. We do not claim wavelength-dependent photodiode phase, nonuniform-generation effects, finite-exponential model identification, or frequency-domain consistency testing as new. The narrower result is a mechanism-blind, calibrated-kernel \emph{warning-before-claim} criterion: in the tested ordinary detector-physics domains, model inadequacy becomes observable at lower measurement precision than a false homogeneous microscopic transport interpretation would require.
\end{abstract}
...
\centering
\begin{tikzpicture}[
box/.style={draw,rounded corners=1.5pt,align=center,text width=3.1cm,minimum height=0.95cm,font=\small},
arr/.style={-{Latex[length=2mm]},line width=.45pt}]
\node[box] (obs) at (0,0) {six calibrated spectral kernels\\$+\ J_m(\omega)$};
\node[box] (one) at (4.4,0) {one-mode null\\$J_m=A+B M_m(r)$};
\node[box] (two) at (8.8,0) {two-mode diagnostic\\$r_1,r_2$};
\node[box] (law) at (13.2,0) {physical root law\\$r_1+r_2\in\mathbb R$, RF-invariant};
\node[box] (warn) at (6.6,-2.0) {$S_{\rm warn}$\\model inadequacy detectable};
\node[box] (claim) at (11.0,-2.0) {$S_{\rm claim}$\\false microscopic claim};
\draw[arr] (obs)--(one); \draw[arr] (one)--(two); \draw[arr] (two)--(law);
\draw[arr] (one)--(warn); \draw[arr] (law)--(warn); \draw[arr] (warn)--node[above,font=\scriptsize]{desired ordering}(claim);
\end{tikzpicture}
\caption{Mechanism-blind warning-before-claim logic. The inverse is asked first whether a low-dimensional transport interpretation is adequate, not which hidden multidimensional mechanism generated the data.}
\label{fig:logic}
\end{figure}
...
\end{equation}
The complex constants $A$ and $B$ absorb wavelength-independent offset and gain. For every trial complex root $r$, $A$ and $B$ are profiled by complex linear least squares, leaving a two-real-parameter nonlinear fit in $r$. This formulation is used rather than assuming that the wavelength-dependent kernels are exact rigid translations.
...
\subsection{Frozen false-claim benchmark}

The warning comparison is made against one transport-specific benchmark that was frozen before the broad Paper-03 screen. It is not re-fit at each geometry. The benchmark is a one-dimensional graded-transport signal used only to define the magnitude and precision of a hypothetical microscopic claim. Its signed reference closure phases and current-step amplitude-SNR requirements are
\begin{table}[H]
\centering
\caption{Frozen false-claim benchmark used throughout the Paper-03 program.}
\label{tab:benchmark}
\begin{tabular}{rrr}
\toprule
RF & reference transport phase & $S_{\rm claim}$\\
\midrule
100 MHz & $-0.011978^\circ$ & 96.1 dB\\
500 MHz & $-0.058727^\circ$ & 82.3 dB\\
1 GHz & $-0.110405^\circ$ & 76.7 dB\\
\bottomrule
\end{tabular}
\end{table}
The benchmark therefore defines the question ``could ordinary device physics imitate a signal of the size and precision that would otherwise be assigned to this microscopic transport mechanism?'' It is not asserted to be a universal detection threshold or a material-independent figure of merit.

\subsection{Statistical warning threshold}

The finite-sample first-family and coplanar tests use the locked noise model
\begin{equation}
n_m=\sigma(\xi_{m,R}+i\xi_{m,I}),
\end{equation}
with independent standard normal quadratures. Define
\begin{equation}
...
\label{eq:snr}
\end{equation}
For all-six one-mode fitting the regular residual has six real degrees of freedom. The nominal test uses two-sided $3\sigma$ significance, $\alpha=0.002699796063260207$, and target power 0.90. The analytic noncentral-$\chi^2$ calculation supplies a central SNR coordinate; the actual parametric bootstrap is evaluated only on the predeclared grid
\begin{equation}
...
The warning and false-claim SNRs are deliberately not identically calibrated statistical tests. $S_{\rm warn}$ is the lowest tested model-rejection SNR achieving 90\% power at $\alpha=0.002699796063260207$, whereas $S_{\rm claim}$ is the pre-existing frozen $3\sigma$ current-step threshold. Requiring both a $3\sigma$-level rejection criterion and 90\% power on the warning side makes the comparison conservative for establishing the warning-before-claim ordering; the reported dB margin should not be interpreted as the difference between two identically constructed hypothesis tests.

\section{Forward models and blind-analysis contract}

\subsection{Finite-contact two-dimensional family}

The first family uses a $16~\mu\mathrm{m}$ lateral width and $7.6~\mu\mathrm{m}$ absorber thickness. A selected top contact and full bottom electrode define both the physical-field problem and a separate selected-electrode weighting-potential problem. The physical bias is 0.30 V. Drift follows the local electric field with a velocity-rolloff prescription, while diffusion is represented by a positive exponentially fitted nearest-neighbor Markov generator. Optional first-order recombination is implemented as independent killing.
...
\label{eq:resolvent}
\end{equation}
At infinite lifetime and DC the discrete construction obeys
\begin{equation}
H(0)=p_{\rm sel}-\phi_w.
\label{eq:ramoid}
\end{equation}
...
The predeclared broad screen spans selected-contact fraction, depletion/space-charge coordinate, diffusion coefficient, lifetime, beam width, and beam offset. A 60-row predeclared screen, with deliberate Block-B overlap at several Block-A anchors, is followed by mechanical S0--S7 point selection and independent numerical refinement before expensive bootstrap testing.
...
The final generic validation replaces the prescribed physical field by a self-consistent semiconductor operating state. For the synthetic single-electron model,
\begin{equation}
-\nabla\!\cdot(\epsilon\nabla\psi)=q(N_D-n),
\label{eq:poisson}
\end{equation}
...
The coarse predeclared screen contains 60 predeclared detector rows and 180 nonzero-RF rows; these are not 60 unique physical coordinates because Block B deliberately overlaps several Block-A anchors. We define an order-one confound as a finite-minus-planar historical phase magnitude at least 0.5 times the frozen reference transport signal. Forty-two RF rows satisfy that criterion. No coarse row simultaneously exhibits an order-one confound and a nonpositive analytic warning margin. Fourteen of the 60 screen rows contain an order-one confound.

The mechanical S0--S7 rules select six unique coordinates for refinement, including the nominal point, maximum confound, warning-boundary coordinates, strongest mismatch, optical-position stress, and weakest still-order-one case. All six selected coordinates pass the fixed refinement gate and retain at least one order-one confound; none develops an analytic hidden-risk row.

The nominal point and two predeclared adversarial points are then evaluated with the finite-sample procedure of Sec.~II.D. Table~\ref{tab:firstfamily} gives all nine RF comparisons. Every cell reaches at least 90\% power before the frozen false-claim SNR.

\begin{table}[H]
\centering
\caption{Finite-sample warning-before-claim result for the nominal and mechanically selected adversarial first-family coordinates.}
\label{tab:firstfamily}
\begin{tabular}{llrrr}
\toprule
Coordinate & RF & warning SNR & claim SNR & margin\\
\midrule
S0 nominal & 100 MHz & 76.545 & 96.1 & 19.555\\
S0 nominal & 500 MHz & 73.137 & 82.3 & \textbf{9.163}\\
S0 nominal & 1 GHz & 65.892 & 76.7 & 10.808\\
R1\_B04 max-confound & 100 MHz & 62.273 & 96.1 & 33.827\\
R1\_B04 max-confound & 500 MHz & 59.923 & 82.3 & 22.377\\
R1\_B04 max-confound & 1 GHz & 56.690 & 76.7 & 20.010\\
R2\_A04 warning-boundary & 100 MHz & 62.360 & 96.1 & 33.740\\
R2\_A04 warning-boundary & 500 MHz & 61.313 & 82.3 & 20.987\\
R2\_A04 warning-boundary & 1 GHz & 57.658 & 76.7 & 19.042\\
\bottomrule
\end{tabular}
\end{table}

The minimum conservative tested margin is 9.16 dB. Thus the broad first family supports Outcome A: ordinary geometry can mimic the targeted transport signal at order-one amplitude, but the calibrated-kernel model inadequacy is detectable first.
...
\begin{tabular}{rrrr}
\toprule
RF & warning SNR & false-claim SNR & warning margin\\
\midrule
...
\subsection{Operating-state, weighting, and reciprocity convergence}
...
The accepted operating-state pair is $51\times39\rightarrow61\times47$. Table~\ref{tab:stagebmesh} gives the convergence coordinates.

\begin{table}[H]
\centering
\caption{Accepted generic Stage-B operating-state/weighting mesh-convergence pair.}
...
\subsection{Aligned six-current observable convergence}

The blind spectral observable required a separate refinement because its sensitivity to finite-contact discretization is stronger than the bulk operating-state metrics. Two earlier confirmation attempts are retained as failures. The identified numerical defect was contact-edge snapping: with arbitrary lateral cell counts the physical selected-contact edge moved by a fraction of a cell between meshes. Before generating the final result, the aligned production pair was frozen as
\begin{equation}
96\times75\ \longrightarrow\ 112\times87,
\end{equation}
for which the physical $x=\pm6~\mu\mathrm m$ contact boundaries lie exactly on cell faces at both resolutions.

The acceptance coordinate is the best complex-affine coarse-to-fine six-current shape residual divided by the fine-grid contrast. For the finite structure the nonzero-RF values are 0.002477, 0.002810, and 0.003143; for the same-physics full-top reference they are 0.003533, 0.003697, and 0.003792. All are below the frozen 0.02 gate.

The historical raw four-color principal-log phase is highly ill-conditioned in this self-consistent case and is therefore diagnostic only. It is neither an observable-convergence acceptance coordinate nor quantitative evidence for the Stage-B warning margin. The earlier raw-phase failures are not retroactively relabeled as passes.

\subsection{Blind six-channel result}

The final Stage-B blind gate is run only after the refined operating-state and aligned six-current numerical gates pass. The analyzer is supplied with the restricted input described in Sec.~III.D. The finite selected-contact structure is compared with a same-physics full-top reference using identical calibrated optical kernels, but the finite/planar mechanism label is not supplied to the blind fit.

At the fine aligned grid the calibrated one-mode contrast-normalized residuals are 0.534, 0.574, and 0.612 at 100 MHz, 500 MHz, and 1 GHz. The two-mode diagnostic reduces them to approximately $6.99\times10^{-5}$, $3.63\times10^{-4}$, and $9.23\times10^{-4}$. The matched two-root sets are numerically stable. Their fine-grid sums are approximately
\begin{equation}
-0.37185+0.00679i,\quad
-0.38445+0.03881i,\quad
-0.42028+0.06833i\quad \mu\mathrm m^{-1},
\end{equation}
for 100 MHz, 500 MHz, and 1 GHz. Each imaginary part exceeds the frozen numerical-separation allowance for a real sum, and every RF pair violates RF invariance.

The predeclared classification is B2-A. Because the historical raw phase is ill-conditioned, we do not use its very large finite-minus-reference mimic ratio to strengthen the quantitative claim. The Stage-B evidence used here is instead the converged six-current shape, strong calibrated-kernel one-mode rejection, stable higher-order fit, physical-root-law failure, and analytic warning ordering.

Because Stage B is a self-consistent forward-model validation rather than a new finite-sample statistical campaign, its warning thresholds are analytic noncentral-$\chi^2$ coordinates. They must not be conflated with the bootstrap thresholds in Tables~\ref{tab:firstfamily} and \ref{tab:coplanar}.
...
\begin{tabular}{rrrr}
\toprule
RF & analytic rejection SNR & false-claim SNR & analytic warning margin\\
\midrule
...
The two-mode diagnostic provides a second defense against false mechanism assignment. In both fixed-field topology families and in Stage B, one additional mode can describe much of the spectral structure, yet the recovered roots fail the homogeneous scalar physical law. Thus ``a compact two-root fit exists'' is not equivalent to ``a homogeneous two-root microscopic transport mechanism is valid.''
...
The candidate contribution is the combination of independently calibrated spectral kernels, mechanism-blind model-order/physical-law falsification, and an explicit comparison between the precision at which model inadequacy becomes rejectable and the precision at which a false homogeneous microscopic transport parameter would otherwise become claim-worthy. The present work validates that warning-before-claim ordering over a broad ordinary geometry family, a materially different topology, and a self-consistent semiconductor forward model. No first-ever or superlative priority claim is made.
...
Fourth, the arbitrary-kernel two-mode fit is a diagnostic rather than a universal theorem. Physical interpretation is made only after numerical root stability and the cross-frequency root law are tested. More complicated ordinary physics can increase effective model order; in that case the appropriate conclusion is higher order/mechanism unresolved rather than forced two-root diagnosis.

Fifth, the tested parameter domains are broad enough to satisfy the predeclared standalone criterion, but they are not exhaustive over all possible detector architectures. A hidden-confound region may exist outside the declared domain. The present statement is therefore domain-bounded: no hidden-risk point was found in the first-family screen, the materially different coplanar topology preserved the safety ordering, and the self-consistent validation did not expose a counterexample.

Finally, the false-claim benchmark is deliberately fixed. The positive margins reported here answer a conditional experimental-design question relative to that frozen microscopic target. They should not be read as a universal dB guarantee for arbitrary transport claims, noise models, or optical kernels.
